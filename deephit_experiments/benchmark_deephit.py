...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--num_durations', default=range(2, 300, 2)
        )
    ...
    fold_results = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    ...
    for fold in tqdm(range(args.cv_folds)):
        ...
        for param in tqdm(params, total=len(params)):
            ...
            for horizon in enumerate(horizons):
                ...
                fold_results[
                    'Fold: {}'.format(fold)
            ]['Durations {}'.format(param['num_durations'])][
    ...
    for fold in fold_results.keys():
        fold_results_ = pd.DataFrame(fold_results[fold])
        ...
        fold_results_.to_csv(
            './fold_results/fold_results_{}_{}_seed_{}_{}.csv'.format(
                args.dataset,
                MODEL_NAME,
                SEED,
                fold.replace(':','_')
